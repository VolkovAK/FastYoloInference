...
def convert(cfg_file_path, weights_file_path, onnx_file_path):
    ...
    # These are the only layers DarkNetParser will extract parameters from. 
    supported_layers = ['net', 'convolutional', 'shortcut', 'route', 'upsample', 'yolo']
    ...
    layers = parser.parse_cfg_file(cfg_file_path)
    ...
    input_width, input_height = 416, 416 # by default
    current_size = input_width
    prev_layer = None
    for layer in layers:
        cur = layers[layer]
        if cur['type'] == 'net':
            input_width = cur['width']
            input_height = cur['height']
        if cur['type'] == 'convolutional':
            current_size = current_size / cur['stride'] # assume that divison will be without remainder
        if cur['type'] == 'upsample':
    ...
    onnx.save(yolov3_model_def, onnx_file_path)
